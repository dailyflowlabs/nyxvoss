      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: product.description ? product.description.substring(0, 250) : 'Official Nyx Voss Merchandise',
...
app.post('/api/merch/order', async (req, res) => {
  try {
    ...
    const orderPayload = {
      ...
      label: `Nyx Voss Order - ${session.id.substring(0, 10)}`,
      line_items: [
        {
...
app.listen(PORT, () => {
  console.log(`Nyx Voss official server running on http://localhost:${PORT}`);
});
